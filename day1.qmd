...
* Why is this projection called _The simplest of all projections_?
* Project `ne_countries` to Plate Carrée, and plot it with `axes=TRUE`. What has changed? (Hint: `st_crs()` accepts a _proj string_ to define a coordinate reference system (CRS); `st_transform()` transforms a dataset to a new CRS.)
